...
public interface IMemberTokenService
{
    ...
    Task<Result<MemberTokenResponse>> GenerateUserToken(User user, QrData qrData, CancellationToken cancellationToken);
    Task<Result<MemberTokenResponse>> GenerateMemberTokenWithPassword(User user, string password, QrData qrData, CancellationToken cancellationToken);
